...
namespace ProTechTiveGear.Controllers
{
    public class ItemsController : Controller
    {
        ...
        // GET: Items/Quantity/5
        public ActionResult Quantity(long? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        [HttpPost]
        public ActionResult Quantity(FormCollection form)
        {
            var id = long.Parse(form["id"]);
            var soluong = Int32.Parse(form["soluong"]);
            var update = db.Items.Find(id);
            update.Quantity = update.Quantity + soluong;
            db.SaveChanges();
            return RedirectToAction("Index");
